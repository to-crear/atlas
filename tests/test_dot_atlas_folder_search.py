...
def test_dot_atlas_folder_search(tmp_path):
    ...
    assert type(AtlasManager(str(dot_atlas_dir)).project_json_path)  == str
   

def test_dot_atlas_folder_search_non_existent(tmp_path):
    with pytest.raises(AtlasManagerError, match='Error saving information to atlas_project.json.'):
